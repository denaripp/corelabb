...
export function HowItWorks() {
  const steps = [
    { title: "Hubungi via WhatsApp", icon: MessageSquareText },
    { title: "Jelaskan masalah Anda", icon: MessageSquare },
    { title: "Analisis kebutuhan", icon: Search },
    { title: "Pengerjaan service", icon: Wrench },
    { title: "Selesai & puas", icon: ThumbsUp },
  ];
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-8 md:gap-4">
            {steps.map((step, idx) => (
              ...
              >
                ...
                <h3 className="font-medium text-white/90 px-2">{step.title}</h3>
              </motion.div>
            ))}
          </div>
        </div>
